...
module BlockGraph
  module Model
    class TxOut < ActiveNodeBase
      ...
      property :output_type, type: Integer
      ...
      alias :oa_output_type :output_type
      ...
      def apply_oa_attributes(oa_out)
        ...
        if self.asset_id.nil? && oa_out.output_type != BlockGraph::Constants::OutputType::MARKER_OUTPUT
          self.output_type = BlockGraph::Constants::OutputType.output_type_label(BlockGraph::Constants::OutputType::UNCOLORED)
        else
          self.output_type = BlockGraph::Constants::OutputType.output_type_label(oa_out.output_type)
        end
        save!
      end
      ...
      def self.import(file_name)
        ...
        self.neo4j_query("USING PERIODIC COMMIT LOAD CSV WITH HEADERS FROM 'file:///#{file_name}.csv' AS row WITH row.value AS value, row.n AS n,
                          ...
                          ON CREATE SET tx.value = toFloat(value), tx.n = toInteger(n), tx.script_pubkey = script_pubkey, tx.asset_quantity = toInteger(asset_quantity), tx.output_type = toInteger(output_type), tx.updated_at = timestamp(), tx.created_at = timestamp()
                          ON MATCH SET tx.value = toFloat(value), tx.n = toInteger(n), tx.script_pubkey = script_pubkey, tx.asset_quantity = toInteger(asset_quantity), tx.output_type = toInteger(output_type), tx.updated_at = timestamp()
                        ")
        CSV.foreach(File.join(self.neo4j_query('CALL dbms.listConfig() yield name,value WHERE name=~"dbms.directories.import" RETURN value').rows.first, "#{file_name}_large.csv"), headers: true) do |csv|
          self.neo4j_query("MERGE (tx:`BlockGraph::Model::TxOut`:`BlockGraph::Model::ActiveNodeBase`
                            {uuid: '#{csv["uuid"]}'})
                            ON CREATE SET tx.value = toFloat(#{csv["value"]}), tx.n = toInteger(#{csv["n"]}), tx.script_pubkey = #{csv["script_pubkey"]}, tx.asset_quantity = toInteger(#{csv["asset_quantity"]}), tx.output_type = toInteger(#{csv["output_type"]}), tx.updated_at = timestamp(), tx.created_at = timestamp()
                            ON MATCH SET tx.value = toFloat(#{csv["value"]}), tx.n = toInteger(#{csv["n"]}), tx.script_pubkey = '#{csv["script_pubkey"]}', tx.asset_quantity = toInteger(#{csv["asset_quantity"]}), tx.output_type = toInteger(#{csv["output_type"]}) tx.updated_at = timestamp()
                          ")
        end
      ...
      def self.import_node(num)
        ...
        self.neo4j_query("USING PERIODIC COMMIT LOAD CSV WITH HEADERS FROM 'file:///tx_outputs#{num_str}.csv' AS row WITH row.uuid AS uuid, row.value AS value,
                          row.n AS n, row.script_pubkey AS script_pubkey, row.asset_quantity AS asset_quantity, row.output_type AS output_type
                          MERGE (tx:`BlockGraph::Model::TxOut`:`BlockGraph::Model::ActiveNodeBase`
                          {
                            uuid: uuid
                          })
